<template>
	<view class="nav">
		<view class="nv_padding" v-if="!!mysNavConfig.navPadding" :style="{ height: navBarHeight,}"></view>
		<view class="bg_box" 
		:style="{
		...
		:class="{ fixed: mysNavConfig.navFixed }">
			...
			<view class="btn_type" :style="{ height: btnHeight }">
				...
				<view class="my_custom_box" :style="{ height: btnHeight }">
					...
					<view class="nv_search_box" v-if="!!mysNavConfig.searchConfig" :style="{
						...
						<image class="nv_icon" src="@/static/images/search.png" mode="aspectFit" />
			...
			<!-- 右边slot -->
			<view class="my_right_icon" :style="{ height: btnHeight }" @click="onRightClick">
				<image v-if="mysNavConfig.rightIconPath"  class="nv_icon"
...
<script>
	...
	/* 默认返回图标路径 */
	const backIconPath = "/static/logo.png";
	export default {
		props: {
			mysNavConfig: {
				type: Object,
